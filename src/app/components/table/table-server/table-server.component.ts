...
export class TableServerComponent extends TableComponent
  ...
  applySpecificFilter(filterValue: any, category: string) {
    ...
    this.keyupTimeout = setTimeout(this.loadDataPage.bind(this), 500);
  }
  ...
  isAllSelected() {
    return (
      this.selection.selected.length === this.tableServerData.dataSubject.value.length
    );
  }
