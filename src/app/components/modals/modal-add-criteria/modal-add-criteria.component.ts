...
@Component({
  ...
  styleUrls: [
    '../modal-fields/modal-fields.component.scss',
    './modal-add-criteria.component.scss',
  ],
  providers: [
...
export class ModalAddCriteriaComponent implements OnInit, OnDestroy {
  ...
  makeForm() {
    ...
    this.fields.forEach((fieldName: string) => {
      ...
      const validators = this.formService.getFieldValidators(
        field.isRequired,
        field.pattern
      );
  ...
  onChanges(): void {
    ...
    this.subscribers.push(
      this.form.get('criteriaType').valueChanges.subscribe((value) => {
        this.criteriaSubList = this.criteriaList.filter((criteria: Criteria) => {
          return (
            criteria.get<string>('target') === value &&
            !notDisplayedCriteria.includes(criteria.get<string>('field'))
          );
        });
  ...
  loadFields() {
    this.criteriaService.get().subscribe((criteria: any) => {
      if (criteria) {
        this.criteriaList = criteria.map((criterion: any) =>
          Criteria.apiToModel(criterion)
        );
        this.onChanges();
  ...
  onSubmit() {
    ...
    this.criteriaList.forEach((option: Criteria) => {
      let field = null;
      if (
        controls.field.value === 'locationType' &&
        controls.value.value === 'camp' &&
        controls.condition.value === '=' &&
        controls.campName.value
      ) {
    ...
    if (controls.field.value === 'currentLocation') {
      if (controls.adm4.value) {
        ...
        const adm4Value = this.location
          .getOptions('adm4')
          .filter((adm: Adm) => adm.get('id') === controls.adm4.value)[0];
        this.criteria.set(
          'value',
          new CriteriaValue(controls.adm4.value, adm4Value.get('name'))
        );
      } else if (controls.adm3.value) {
        this.criteria.set('field', 'currentAdm3');
        const adm3Value = this.location
          .getOptions('adm3')
          .filter((adm: Adm) => adm.get('id') === controls.adm3.value)[0];
        this.criteria.set(
          'value',
          new CriteriaValue(controls.adm3.value, adm3Value.get('name'))
        );
      } else if (controls.adm2.value) {
        this.criteria.set('field', 'currentAdm2');
        const adm2Value = this.location
          .getOptions('adm2')
          .filter((adm: Adm) => adm.get('id') === controls.adm2.value)[0];
        this.criteria.set(
          'value',
          new CriteriaValue(controls.adm2.value, adm2Value.get('name'))
        );
      } else if (controls.adm1.value) {
        this.criteria.set('field', 'currentAdm1');
        const adm1Value = this.location
          .getOptions('adm1')
          .filter((adm: Adm) => adm.get('id') === controls.adm1.value)[0];
        this.criteria.set(
          'value',
          new CriteriaValue(controls.adm1.value, adm1Value.get('name'))
        );
      }
      this.criteria.set(
        'condition',
        this.criteria
          ...
          .filter(
            (option: CriteriaCondition) => option.get('name') === controls.condition.value
          )[0]
      );
    } else if (
      controls.field.value === 'locationType' &&
      ...
      const campValue = this.campList.filter(
        (camp: Camp) => camp.get('id') === controls.campName.value
      )[0];
      this.criteria
        .set('value', new CriteriaValue(controls.campName.value, campValue.get('name')))
        .set(
          'condition',
          this.criteria
            .getOptions('condition')
            .filter((option: CriteriaCondition) => option.get<string>('name') === '=')[0]
        );
    } else {
      this.criteria.set(
        'condition',
      ...
      if (
        ...
        controls.field.value === 'headOfHouseholdGender'
      ) {
        const genderValue = this.criteria.genders.filter(
          (gender: Gender) => gender.get('id') === value
        )[0];
        this.criteria.set('value', new CriteriaValue(value, genderValue.get('name')));
      } else if (controls.field.value === 'livelihood') {
        const livelihoodValue = this.livelihoods.filter(
          (livelihood: Livelihood) => livelihood.get('id') === value
        )[0];
        this.criteria.set('value', new CriteriaValue(value, livelihoodValue.get('name')));
      } else if (controls.field.value === 'residencyStatus') {
        const residencyStatusValue = this.residencyStatuses.filter(
          (residencyStatus: ResidencyStatus) => residencyStatus.get('id') === value
        )[0];
        this.criteria.set(
          'value',
          new CriteriaValue(value, residencyStatusValue.get('name'))
        );
      } else if (controls.field.value === 'locationType') {
        const locationTypeValue = this.locationTypes.filter(
          (locationType: HouseholdLocationType) => locationType.get('id') === value
        )[0];
        this.criteria.set(
          'value',
