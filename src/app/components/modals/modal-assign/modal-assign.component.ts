...
export class ModalAssignComponent implements OnInit {
  ...
  public voucherPasswordControl = new FormControl('', [
    Validators.required,
  ...
  getDistributions() {
    this.distributionService
      .getQrVoucherByProject(this.projectControl.value)
      .subscribe((response) => {
        if (response) {
          // this.distributions = this.distributionClass.formatArray(response);
          this.distributions = response.map((distribution: any) =>
            Distribution.apiToModel(distribution)
          );
  ...
  getBeneficiaries() {
    this.distributionService
      .getAssignableBeneficiaries(this.distributionControl.value)
      .subscribe((response) => {
        if (response) {
          // this.beneficiaries = this.beneficiariesClass.formatArray(response);
          this.beneficiaries = response.map((distributionBeneficiary: any) =>
            Beneficiary.apiToModel(distributionBeneficiary.beneficiary)
          );
          this.beneficiaryControl.setValue(null);
        } else {
          this.beneficiaries = [];
  ...
  assignBooklet() {
    ...
    const assignObservable = this.bookletService
      .assignBenef(
        bookletId,
        this.beneficiaryControl.value,
        this.distributionControl.value
      )
      .pipe(finalize(() => (this.loadingAssignation = false)));

    if (this.voucherPasswordControl.value) {
      const passwordObservable = this.bookletService.setPassword(
        this.bookletQRCode,
        this.voucherPasswordControl.value
      );
      forkJoin(assignObservable, passwordObservable).subscribe(
        (_: any) => {
  ...
  getData() {
    if (this.projectControl.value === 0) {
      ...
      if (!this.data.distribution || !this.data.project) {
        this.distributionName = this.distributions
          .filter(
            (distribution: Distribution) =>
              distribution.get('id') === this.distributionControl.value
          )[0]
          .get('name');
      }
      if (!this.data.beneficiary) {
        this.beneficiaryName = this.beneficiaries
          .filter(
            (beneficiary: Beneficiary) =>
              beneficiary.get('id') === this.beneficiaryControl.value
          )[0]
          .get('localFullName');
      }
    }
