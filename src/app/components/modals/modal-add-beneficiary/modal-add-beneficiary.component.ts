...
export class ModalAddBeneficiaryComponent implements OnInit {
  ...
  selectedBeneficiariesControl = new FormControl(
    [],
  ...
  getProjectBeneficiaries() {
    ...
    this.beneficiariesService
      .getAllFromProject(this.data.distribution.get('project').get('id'), target)
      .subscribe((allBeneficiaries) => {
        if (allBeneficiaries) {
          this.beneficiaryList = allBeneficiaries.map((beneficiary: any) =>
            Beneficiary.apiToModel(beneficiary)
          );
        } else {
          this.cacheService
            .get(
              AsyncacheService.PROJECTS +
                '_' +
                this.data.distribution.get('project').get('id') +
                '_beneficiaries'
            )
            .subscribe((beneficiaries) => {
              if (beneficiaries) {
                this.beneficiaryList = beneficiaries.map((beneficiary: any) =>
                  Beneficiary.apiToModel(beneficiary)
                );
              }
            });
