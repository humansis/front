...
@Component({
  ...
  styleUrls: [
    ...
    './modal-add-commodity.component.scss',
  ],
})
export class ModalAddCommodityComponent implements OnInit {
  ...
  ngOnInit() {
    this.organizationServicesService
      .getServiceStatus('wing')
    ...
    this.currencies = this.formService.pushLocalCurrencyOnTop(
      this.currencies,
      this.localCurrency
    );
  }
  ...
  loadTypes(modalityId) {
    if (modalityId) {
      this.commodityService.fillTypeOptions(
        this.commodity,
        modalityId,
  ...
  getUnit(): string {
    const name =
      this.commodity.getOptions('modalityType') && this.form.controls.modalityType.value
        ? this.commodity
            .getOptions('modalityType')
            .filter(
              (modalityType: ModalityType) =>
                modalityType.get('id') === this.form.controls.modalityType.value
            )[0]
            .get<string>('name')
        : null;
    switch (name) {
  ...
  onSubmit() {
    for (const field of this.fields) {
      if (
        ...
        this.commodity.fields[field].kindOfField === 'SingleSelect'
      ) {
        this.commodity.set(
          field,
