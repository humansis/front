...
export class ModalFieldsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.modalType === 'Add') {
      this.objectFields = Object.keys(this.objectInstance.fields).filter(
        (fieldName: string) => this.objectInstance.fields[fieldName].isSettable
      );
    } else {
    ...
    if (this.objectInstance.get('location') && this.modalType !== 'Details') {
      this.initialAdmValues = {
        adm1: this.objectInstance.get('location').get('adm1')
          ? this.objectInstance.get('location').get('adm1').get('id')
          : null,
        adm2: this.objectInstance.get('location').get('adm2')
          ? this.objectInstance.get('location').get('adm2').get('id')
          : null,
        adm3: this.objectInstance.get('location').get('adm3')
          ? this.objectInstance.get('location').get('adm3').get('id')
          : null,
        adm4: this.objectInstance.get('location').get('adm4')
          ? this.objectInstance.get('location').get('adm4').get('id')
          : null,
      };
    }
  ...
  makeForm() {
    ...
    const filteredObjectFields = this.objectFields.filter(
      (fieldName: string) =>
        this.modalType === 'Details' ||
        (fieldName !== 'adm1' &&
          fieldName !== 'adm2' &&
    ...
    this.form = this.formService.makeForm(
      this.objectInstance,
  ...
  onChanges(): void {
    const triggeringFieldsNames = this.objectFields.filter(
      (fieldName) => this.objectInstance.fields[fieldName].isTrigger === true
    );
    triggeringFieldsNames.forEach((fieldName) => {
      this.form.get(fieldName).valueChanges.subscribe((value) => {
        this.objectInstance = this.objectInstance.fields[fieldName].triggerFunction(
          this.objectInstance,
          value,
          this.form
        );
    ...
    multipleSelects.forEach((fieldName) => {
      this.form.get(fieldName).valueChanges.subscribe((value) => {
        if (
          this.objectInstance.fields[fieldName].maxSelectionLength &&
        ...
        if (field.isPassword && !this.form.controls[fieldName].value) {
          ...
          if (this.form.controls[fieldName].value) {
            if (childrenField.fields[childrenFieldName].kindOfField === 'SingleSelect') {
              childrenField.set(
                childrenFieldName,
                childrenField.getOptions(childrenFieldName).filter((option) => {
          ...
          if (this.form.controls[fieldName].value) {
            this.form.controls[fieldName].value.forEach((optionId) => {
              const selectedOption = this.objectInstance
          ...
          field.kindOfField === 'SingleSelect'
        ) {
          this.objectInstance.set(
            fieldName,
            this.objectInstance.getOptions(fieldName).filter((option) => {
              return option.get('id') === this.form.controls[fieldName].value;
            })[0]
          );
        } else if (
          this.form.controls[fieldName].value !== null &&
          this.form.controls[fieldName].value !== undefined
        ) {
  ...
  onFileChange(field: FileModelField, event) {
    if (event.target.files.length > 0) {
      ...
      const fileField = this.objectFields.filter(
        (fieldName: string) =>
          this.objectInstance.fields[fieldName].kindOfField === 'File'
      )[0];
