...
@Component({
  ...
  styleUrls: [
    './adm-form.component.scss',
    '../modals/modal-fields/modal-fields.component.scss',
  ],
})
export class AdmFormComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    this.form.addControl(
      this.formName['adm1'],
      new FormControl(adm1Id, [Validators.required])
    );
    ...
    if (!this.adm1Subscription) {
      this.adm1Subscription = this.locationService
        .fillAdm1Options(this.location)
        .subscribe((filledLocation0: Location) => {
          ...
          if (adm1Id && !this.adm2Subscription) {
            this.adm2Subscription = this.locationService
              .fillAdm2Options(this.location, adm1Id)
              .subscribe((filledLocation1: Location) => {
                this.location = filledLocation1;
                if (adm2Id && !this.adm3Subscription) {
                  this.adm3Subscription = this.locationService
                    .fillAdm3Options(this.location, adm2Id)
                    .subscribe((filledLocation2: Location) => {
                      this.location = filledLocation2;
                      if (adm3Id && !this.adm4Subscription) {
                        this.adm4Subscription = this.locationService
                          .fillAdm4Options(this.location, adm3Id)
                          .subscribe(
                            (filledLocation3: Location) =>
