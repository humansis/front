...
export class Criteria extends CustomModel {
  ...
  public static apiToModel(criteriaFromApi: any): Criteria {
    ...
    newCriteria.set(
      'target',
      criteriaFromApi.target ? criteriaFromApi.target.toLowerCase() : null
    );
    ...
    newCriteria.set(
      'condition',
      condition ? new CriteriaCondition(null, condition) : null
    );
    const value = criteriaFromApi.value_string;
    if (criteriaFromApi.field_string === 'gender' && value) {
      const genderValue = newCriteria.genders.filter(
  ...
  public modelToApi(): ApiCriteria {
    ...
    return {
      ...
      target: this.fields.target.formatForApi()
        ? titleCase.transform(this.fields.target.formatForApi())
        : null,
