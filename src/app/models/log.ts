...
export class Log extends CustomModel {
  ...
  public static apiToModel(logFromApi: any): Log {
    ...
    try {
      ...
      // To filter bad logs
      if (
        /.+\/households\/get\/.+/.test(url) ||
        /.+\/export/.test(url) ||
        /.+\/location.+/.test(url) ||
        /.+\/distributions\/criteria\/project\/\d+\/number/.test(url) ||
        /.+\/login.+/.test(url) ||
        /.+\/distributions\/beneficiaries\/project\/\d+/.test(url) ||
        /.+\/indicators/.test(url) ||
        /.+\/deactivate-booklets/.test(url) ||
        /.+\/booklets-print/.test(url) ||
        /.+\/vouchers\/scanned/.test(url)
      ) {
        return null;
      }
      ...
      // Action and Object
      const controller = /^\w+\\\w+\\(\w+)Controller::(\w+)Action$/g.exec(
        logFromApi.controller
      );
      if (controller) {
        controller.shift();

        newLog.set('objectOfAction', controller[0]);
        const action = controller[1]
          .split(/(?=[A-Z])/)
          .join('_')
          .toLowerCase();
        ...
        if (
          ...
          (url.includes('project') || url.includes('distribution')) &&
        ...
        if (action.includes('update') || action.includes('create')) {
          if (
            controller[0] === 'Household' ||
            controller[0] === 'Beneficiary' ||
            controller[0] === 'Vendor' ||
            controller[0] === 'Donor'
          ) {
            const details = /name":"(.*?)".+/g.exec(request);
            if (details && details.length > 0) {
              newLog.set(
                'details',
                newLog.language['log_' + controller[0].toLowerCase()] + ': ' + details[1]
              );
            }
          } else if (controller[0] === 'CountrySpecific') {
            newLog.set(
              'details',
              newLog.language.log_field + ': ' + /field":"(.*?)"/.exec(request)[1]
            );
          } else if (controller[0] === 'Booklet') {
            if (action.includes('update')) {
              detailString =
                newLog.language.log_code +
                ': ' +
                /code":"(.+?)".+/.exec(request)[1] +
                '\n';
            }
            newLog.set(
              'details',
              detailString +
                newLog.language.log_value +
                ': ' +
                /values":\["(.+?)".+/.exec(request)[1]
            );
          } else {
            const details = /name":"(.*?)".+/g.exec(request);
            if (details && details.length > 0) {
          ...
          newLog.set(
            'details',
            newLog.language.log_old_id + ': ' + /(\d+)/g.exec(url)[1]
          );
        } else if (
          controller[0] === 'Voucher' ||
          action.includes('post') ||
          action.includes('relief') ||
