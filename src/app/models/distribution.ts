...
export class Distribution extends CustomModel {
  ...
  public static apiToModel(distributionFromApi: any): Distribution {
    ...
    newDistribution.set(
      'type',
      distributionFromApi.type >= 0
        ? newDistribution
            .getOptions('type')
            .filter(
              (option: DistributionType) =>
                distributionFromApi.type.toString() === option.get('id')
            )[0]
        : null
    );
    ...
    newDistribution.set(
      'location',
      distributionFromApi.location
        ? Location.apiToModel(distributionFromApi.location)
        : null
    );
    newDistribution.set(
      'project',
      distributionFromApi.project ? Project.apiToModel(distributionFromApi.project) : null
    );

    newDistribution.fields.location.tooltip = (value: Location) =>
      value.getLocationName();
    newDistribution.fields.location.displayTableFunction = (value: Location) =>
      value.getPreciseLocationName();
    newDistribution.fields.location.displayModalFunction = (value) =>
      value.getLocationName();
    newDistribution.fields.distributionBeneficiaries.displayTableFunction = (value) =>
      value.length;
    newDistribution.fields.commodities.displayTableFunction = (value) => value;
    newDistribution.fields.project.displayTableFunction = (value: Project) =>
      value.get('name');

    newDistribution.set(
      'commodities',
      distributionFromApi.commodities
        ? distributionFromApi.commodities.map((commodity: any) =>
            Commodity.apiToModel(commodity)
          )
        : null
    );
    ...
    newDistribution.set(
      'selectionCriteria',
      distributionFromApi.selection_criteria
        ? distributionFromApi.selection_criteria.map((criteria: any) =>
            Criteria.apiToModel(criteria)
          )
        : []
    );
    ...
    if (distributionFromApi.distribution_beneficiaries) {
      newDistribution.set(
        ...
        distributionFromApi.distribution_beneficiaries.map(
          (distributionBeneficiary: any) =>
            DistributionBeneficiary.apiToModel(
              distributionBeneficiary,
              distributionFromApi.id
            )
        )
      );
    }
