...
export class User extends CustomModel {
  ...
  public static apiToModel(userFromApi: any): User {
    ...
    newUser.set(
      ...
      userFromApi.roles
        ? newUser
            .getOptions('rights')
            .filter((role: Role) => role.get('id') === userFromApi.roles[0])[0]
        : null
    );
    const rights = newUser.get('rights')
      ? newUser.get<CustomModel>('rights').get<string>('id')
      : null;
    ...
    if (
      rights === 'ROLE_PROJECT_MANAGER' ||
      rights === 'ROLE_PROJECT_OFFICER' ||
      rights === 'ROLE_FIELD_OFFICER' ||
      rights === 'ROLE_ENUMERATOR'
    ) {
    ...
    if (userFromApi.countries && userFromApi.countries.length) {
      ...
      const allCountries = userFromApi.projects
        .filter((project: any) => !project.project.archived)
        .map((project) => project.project.iso3);
      countries = allCountries.filter(
        (iso3, index) => allCountries.indexOf(iso3) === index
      );
    }
      ...
      countries
        ? countries.map((countryFromApi: any) => {
            return newUser.getOptions('countries').filter((country: Country) => {
              // TODO : clean up
              const formattedCountryFromApi = countryFromApi.iso3
                ? countryFromApi.iso3
                : countryFromApi;
  ...
  public modelToApi(): Object {
    ...
    const rights = this.get('rights')
      ? this.get<CustomModel>('rights').get<string>('id')
      : null;
