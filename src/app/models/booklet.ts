...
export class Booklet extends CustomModel {
  ...
  public static apiToModel(bookletFromApi): Booklet {
    ...
    newBooklet.set(
      'project',
      bookletFromApi.project ? Project.apiToModel(bookletFromApi.project) : undefined
    );
    ...
    newBooklet.set(
      ...
      bookletFromApi.currency
        ? newBooklet
            .getOptions('currency')
            .filter(
              (currency: Currency) => currency.get('name') === bookletFromApi.currency
            )[0]
        : null
    );
    ...
    newBooklet.set(
      'vouchers',
      bookletFromApi.vouchers.map((voucher: any) => Voucher.apiToModel(voucher))
    );
    bookletFromApi.vouchers.forEach((voucher) => {
      individualValues.push(voucher.value);
    });
    const unique = individualValues.filter(
    ...
    newBooklet.set(
      ...
      bookletFromApi.distribution_beneficiary
        ? Beneficiary.apiToModel(bookletFromApi.distribution_beneficiary.beneficiary)
        : null
    );
    newBooklet.set(
      'distribution',
    ...
    newBooklet.fields.value.displayValue =
      newBooklet.getTotalValue() + ' ' + newBooklet.get('currency').get('name');

    // No need to format the date, it is a voucher's date so already formatted
    newBooklet.set('usedAt', newBooklet.getUsedAt());

    newBooklet.fields.beneficiary.displayTableFunction = (value: Beneficiary) =>
      value ? value.get('localFullName') : null;
    newBooklet.fields.beneficiary.displayModalFunction = (value: Beneficiary) =>
      value ? value.get('localFullName') : null;
    newBooklet.fields.distribution.displayTableFunction = (value: Distribution) =>
      value ? value.get('name') : null;
    newBooklet.fields.distribution.displayModalFunction = (value: Distribution) =>
      value ? value.get('name') : null;

    if (bookletFromApi.password) {
      newBooklet.fields.definePassword.title =
        newBooklet.language.booklet_update_password;
    }
    ...
    newBooklet.setOptions(
      'currency',
      currencies.map((currency) => new Currency(currency.id, currency.name))
    );
  ...
  public modelToApi(): Object {
    const password = this.get('password')
      ? CryptoJS.SHA1(this.get('password')).toString(CryptoJS.enc.Base64)
      : null;
  ...
  public getUsedAt(): Date {
    ...
    let total = 0;
    if (
      this.get<CustomModel>('status').get<string>('id') === '2' ||
      this.get<CustomModel>('status').get<string>('id') === '3'
    ) {
      this.get<Voucher[]>('vouchers').forEach((voucher: Voucher) => {
      ...
      text =
        numberUsed + '/' + total + ' (' + datePipe.transform(date, 'dd-MM-yyyy') + ')';
    } else {
      this.get<Voucher[]>('vouchers').forEach((voucher: Voucher) => {
        total += 1;
