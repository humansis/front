  ...
  public fields = {
    ...
    residencyStatus: new SingleSelectModelField({
      ...
      options: [
        new ResidencyStatus(
          'refugee',
          this.language.beneficiary_residency_status_refugee
        ),
        new ResidencyStatus('IDP', this.language.beneficiary_residency_status_idp),
        new ResidencyStatus(
          'resident',
          this.language.beneficiary_residency_status_resident
        ),
      ],
      bindField: 'name',
      apiLabel: 'id',
      value: new ResidencyStatus(
        'resident',
        this.language.beneficiary_residency_status_resident
      ),
    }),
  ...
  public static apiToModel(beneficiaryFromApi: any): Beneficiary {
    ...
    newBeneficiary.set(
      ...
      beneficiaryFromApi.residency_status
        ? newBeneficiary
            ...
            .filter(
              (option: ResidencyStatus) =>
                option.get<string>('id').toLowerCase() ===
                beneficiaryFromApi.residency_status.toLowerCase()
            )[0]
    ...
    newBeneficiary.set(
      ...
      beneficiaryFromApi.gender >= 0
        ? newBeneficiary
            ...
            .filter(
              (option: Gender) =>
                option.get('id') === beneficiaryFromApi.gender.toString()
            )[0]
        : null
    );
    ...
    newBeneficiary.set(
      ...
      beneficiaryFromApi.national_ids && beneficiaryFromApi.national_ids.length !== 0
        ? beneficiaryFromApi.national_ids.map((nationalId) =>
            NationalId.apiToModel(nationalId)
          )
        : [new NationalId()]
    );
    ...
    newBeneficiary.set(
      ...
      beneficiaryFromApi.vulnerability_criteria
        ? beneficiaryFromApi.vulnerability_criteria.map((criteria) =>
    ...
    newBeneficiary.set(
      'profile',
      beneficiaryFromApi.profile
        ? Profile.apiToModel(beneficiaryFromApi.profile)
        : new Profile()
    );
    ...
    newBeneficiary.fields.vulnerabilities.displayModalFunction = (value) =>
      value
        .map((vulnerability: VulnerabilityCriteria) =>
          pipe.transform(vulnerability.get('name'))
        )
        .join(', ');
    newBeneficiary.fields.phones.displayTableFunction = (value) =>
      value.map((phone: Phone) => phone.get('number')).join(', ');
    newBeneficiary.fields.phones.displayModalFunction = (value) =>
      value.map((phone: Phone) => phone.get('number')).join(', ');
    newBeneficiary.fields.nationalIds.displayModalFunction = (value) =>
      value.map((nationalId: NationalId) => nationalId.get('number')).join(', ');
    newBeneficiary.fields.nationalIds.displayTableFunction = (value) =>
      value
        ? value.map((nationalId: NationalId) => nationalId.get('number')).join(', ')
        : null;
    ...
    if (beneficiaryFromApi.referral) {
      ...
      newBeneficiary.set(
        ...
        newBeneficiary
          ...
          .filter(
            (option: BeneficiaryReferralType) =>
              option.get('id') === beneficiaryFromApi.referral.type
          )[0]
      );
