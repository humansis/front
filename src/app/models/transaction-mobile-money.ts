...
export class TransactionMobileMoney extends DistributionBeneficiary {
  ...
  public static apiToModel(
    ...
    if (
      ...
      Number.isInteger(transactions[0].transaction_status)
    ) {
      ...
      if (lastTransaction) {
        newDistributionBeneficiary.set(
          'message',
          lastTransaction.message ? lastTransaction.message : ''
        );
        if (lastTransaction.transaction_status === 0) {
          newDistributionBeneficiary.updateState('0');
        } else if (
          lastTransaction.transaction_status === 1 &&
          !lastTransaction.money_received
        ) {
    ...
    // It means it comes from the cache, already "formatted"
      ...
      newDistributionBeneficiary.set(
        'idTransaction',
        distributionBeneficiaryFromApi.transaction_id
      );
  ...
  public updateState(stateId: string) {
    const state: State = this.getOptions('state').filter(
      (option: State) => option.get<string>('id') === stateId
    )[0];
