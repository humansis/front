...
export class FormService {
  ...
  isEmpty(field, type) {
    ...
    if (field.kindOfField === 'Object' || field.kindOfField === 'MultipleObject') {
      const displayedValue =
        type === 'modal'
          ? field.displayModalFunction(field.value)
      ...
      field.kindOfField === 'MultipleSelect'
    ) {
      ...
      if (field.kindOfField === 'MultipleSelect') {
        ...
        if (!field.isPassword) {
          if (field.childrenFieldName === 'adm1') {
            ...
            field.childrenFieldName === 'adm2' ||
            field.childrenFieldName === 'adm3' ||
            field.childrenFieldName === 'adm4'
          ) {
            ...
            if (
              ...
              value = childrenField.get(childrenFieldName)
                ? childrenField.get(childrenFieldName).get('id')
                : null;
            } else {
  ...
  initLocation(object: CustomModel) {
    ...
    this.locationService
      .fillAdm1Options(location)
      .subscribe((filledLocationAdm1: Location) => {
        if (
          !object.get('location').get('adm1') ||
          !object.get('location').get('adm1').get('id')
        ) {
          object.set('location', filledLocationAdm1);
          return;
        }
        const adm1Id = object
          .get<CustomModel>('location')
          .get<CustomModel>('adm1')
          .get<number>('id');
        this.form.controls['adm1'].setValue(adm1Id);
        this.locationService
          .fillAdm2Options(filledLocationAdm1, adm1Id)
          .subscribe((filledLocationAdm2: Location) => {
            if (
              !object.get('location').get('adm2') ||
              !object.get('location').get('adm2').get('id')
            ) {
            ...
            const adm2Id = object
              .get<CustomModel>('location')
              .get<CustomModel>('adm2')
              .get<number>('id');
            this.form.controls['adm2'].setValue(adm2Id);
            this.locationService
              .fillAdm3Options(filledLocationAdm2, adm2Id)
              .subscribe((filledLocationAdm3: Location) => {
                if (
                  !object.get('location').get('adm3') ||
                  !object.get('location').get('adm3').get('id')
                ) {
                ...
                const adm3Id = object
                  .get<CustomModel>('location')
                  .get<CustomModel>('adm3')
                  .get<number>('id');
                this.form.controls['adm3'].setValue(adm3Id);
                this.locationService
                  .fillAdm4Options(filledLocationAdm3, adm3Id)
                  .subscribe((filledLocationAdm4: Location) => {
                    object.set('location', filledLocationAdm4);
                    if (!object.get('location').get('adm4')) {
                      return;
                    }
                    this.form.controls['adm4'].setValue(
                      object.get('location').get('adm4').get('id')
                    );
                  });
              });
          });
