...
export class ModalService {
      ...
      const subscription = dialogRef.afterClosed().subscribe((closeMethod: any) => {
        ...
        if (closeMethod === 'Add') {
          ...
          this.referedClassService
            .create(this.referedClassInstance.modelToApi())
            .subscribe((response) => {
          ...
          this.isLoading.next();
          this.addBeneficiary(
            closeMethod.beneficiaries,
            closeMethod.justification,
            dialogDetails.distribution
          );
  ...
  updateElement(updateElement) {
    ...
    this.referedClassService.update(apiUpdateElement['id'], apiUpdateElement).subscribe(
      (response: any) => {
        // If the response is null, it means we are offline and the request was stored, not really made
        if (response) {
  ...
  deleteElement(deleteElement: CustomModel) {
    if (deleteElement instanceof Beneficiary) {
      this.referedClassService
        .delete(deleteElement.get('id'), deleteElement.get('distributionId'))
        .subscribe(
          (response: any) => {
            this.isCompleted.next(true);
            this.dataSubject.next(response);
      ...
      this.referedClassService.delete(deleteElement.get('id')).subscribe(
        (response: any) => {
          this.isCompleted.next(true);
          this.dataSubject.next(response);
        },
  ...
  addBeneficiary(
    ...
    const beneficiariesArray = beneficiaries.map((beneficiary: Beneficiary) =>
      beneficiary.modelToApi()
    );
    this.referedClassService
      .add(distribution.get('id'), beneficiariesArray, justification)
      .subscribe(
        (success) => {
          this.isCompleted.next(true);
        ...
        (error) => {
          this.isCompleted.next(false);
          this.snackbar.error(
            error.error ? error.error : this.language.distribution_beneficiary_not_added
          );
        }
      );
  }
  ...
  deleteMany(ids: Array<number>) {
    this.referedClassService.deleteMany(ids).subscribe(
      (response: any) => {
        this.isCompleted.next(true);
        this.dataSubject.next(response);
      },
