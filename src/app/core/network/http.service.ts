...
export class HttpService {
    ...
    if (url.includes(URL_BMS_API, 0)) {
      ...
      switch (true) {
        ...
        case url.substring(0, 24) === '/distributions/projects/':
          return (
            AsyncacheService.DISTRIBUTION_PROJECT +
            '_' +
            url.split('/distributions/projects/')[1]
          );
        case /\/distributions\/\d+\/beneficiaries/.test(url):
          this.save = false;
          return (
            AsyncacheService.DISTRIBUTIONS + '_' + url.split('/')[2] + '_beneficiaries'
          );
  ...
  canBeOfflineRequest(url: string) {
    ...
    if (url.includes(URL_BMS_API, 0)) {
      ...
      if (
        url.substring(0, 11) === '/indicators' ||
        url === '/households/get/all' ||
        url.match(regex)
      ) {
        filtered = false;
      }
        ...
        map(([result, cacheData]) => {
          ...
          if (
            (Array.isArray(result) &&
              ...
              JSON.stringify(result) !== JSON.stringify(cacheData)) ||
            !url.match(regex) ||
            (!(Array.isArray(result) && Array.isArray(cacheData)) && result !== cacheData)
          ) {
            return result;
          } else {
            return cacheData;
  ...
  forceDataInCache(method, url, body?) {
    ...
    if (itemKey) {
      ...
      this.cacheService.get(itemKey).subscribe((result) => {
