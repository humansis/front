...
export class MapService {
  ...
  addDistributions(distributions: Array<Distribution>) {
    ...
    const admLayers = LeafletOmnivore.kml(
      'assets/maps/map_' + country.fields.id.value.toLowerCase() + '.kml'
    ).on('ready', () => {
      ...
      distributions.forEach((distribution: Distribution) => {
        ...
        admLayers.eachLayer((layer: any) => {
          ...
          if (
            this.compareAdmToMapDistribution(
              kmlAdm,
  ...
  private addLegend() {
    new Legend(this.distributionMarkerService.generateMarker(false)).legend.addTo(
      this.map
    );
  }
}
