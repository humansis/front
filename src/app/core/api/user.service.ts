...
export class UserService extends CustomModelService {
  ...
  public updatePassword(user: User, oldPassword: any, newPassword: any) {
    return new Promise<void>((resolve, reject) => {
      this.authenticationService.requestSalt(user.get<string>('username')).subscribe(
        (success) => {
          const getSalt = success as SaltInterface;
          const saltedOldPassword = this.wsseService.saltPassword(
            getSalt.salt,
            oldPassword
          );
          const saltedNewPassword = this.wsseService.saltPassword(
            getSalt.salt,
            newPassword
          );
          this.requestPasswordChange(parseInt(user.get('id'), 10), {
            oldPassword: saltedOldPassword,
            newPassword: saltedNewPassword,
          }).subscribe(
            (data) => {
  ...
  public hasRights(action: string, condition: boolean = true) {
    ...
    const userRights =
      rightsHierarchy[this.currentUser.get<CustomModel>('rights').get<string>('id')];
