...
export class AuthenticationService {
  ...
  login(username: string, password: string) {
    return this.requestSalt(username).pipe(
      switchMap((saltInterface: SaltInterface) => {
        const saltedPassword = this._wsseService.saltPassword(
          saltInterface.salt,
          password
        );
  ...
  sendSMS(body: any, options: any) {
    return this.http
      .post('https://api.sms.test.humanitarian.tech/api/order/sms', body, options)
      .pipe(mapTo(null));
  }
  ...
  public createSaltedPassword(body: any, salt: any) {
    const saltedPassword = body.password
      ? this._wsseService.saltPassword(salt.salt, body.password)
      : null;
