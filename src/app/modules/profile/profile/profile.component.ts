...
export class ProfileComponent implements OnInit {
  ...
  formatUserFields() {
    ...
    this.userTwoFA = this.twoFAService
      ? this.actualUser.get('twoFactorAuthentication')
      : false;
    if (
      this.actualUser.get<string>('phonePrefix') &&
      this.actualUser.get('phoneNumber')
    ) {
      this.canTwoFA = true;
    }
  }
  ...
  onPhoneSubmit(): void {
    if (!this.phoneForm.value.phoneNumber) {
      ...
      this.actualUser.set(
        'phonePrefix',
        this.formatPhonePrefix(this.phoneForm.value.phonePrefix, this.countryId)
      );
      this.actualUser.set('phoneNumber', this.phoneForm.value.phoneNumber);
      this.actualUser.set('password', null);

      this.userService
        .update(this.actualUser.get('id'), this.actualUser.modelToApi())
        .subscribe(
          (data) => {
    ...
    if (prefix) {
      if (/([A-Z])+/.test(prefix)) {
        return this.countryCodesList
          .filter((element) => element === prefix)[0]
          .split('- ')[1];
      } else {
        return prefix;
      }
      ...
      phoneCode = this.countryCodesList
        .filter((element) => element.split(' -')[0] === countryCode)[0]
        .split('- ')[1];
