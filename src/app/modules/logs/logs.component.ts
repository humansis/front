...
export class LogsComponent implements OnInit, OnDestroy {
  ...
  public displayedTable: TableComponent;
  ...
  selectTab(tab: string): void {
    ...
    if (this.logs) {
      this.filteredLogList = this.logs.filter(
        (log: Log) => log.get<string>('tabName') === tab
      );
  ...
  createGraphs() {
    const sortedLogs = this.logs.sort(
      (log1: Log, log2: Log) =>
        log1.get<Date>('date').getTime() - log2.get<Date>('date').getTime()
    );
    ...
    const statusGraph = this.setupGraph('pie', this.language.log_status_rate_title);
    const dayRequestGraph = this.setupGraph(
      'line',
      this.language.log_requests_day_title,
      this.language.log_time,
      this.language.log_requests
    );
    const activeUsersGraph = this.setupGraph(
      'bar',
      this.language.log_active_users_title,
      this.language.log_users,
      this.language.log_requests
    );
    this.requestsKHM = this.setupGraph(
      'line',
      this.language.log_requests + ' KHM',
      this.language.log_time,
      this.language.log_requests
    );
    this.requestsSYR = this.setupGraph(
      'line',
      this.language.log_requests + ' SYR',
      this.language.log_time,
      this.language.log_requests
    );

    const status = [
      this.language.log_status_200,
      this.language.log_status_300,
      this.language.log_status_400,
      this.language.log_status_401,
      this.language.log_status_403,
      this.language.log_status_404,
    ];
    ...
    for (
      const date = new Date(oldestDay.getTime());
      date <= latestDay;
      ...
      requestGraphs.forEach((graph: any) => {
        graph.values[formatDatePipe] = [
          {
            date: formatDatePipe,
            name: this.language.log_requests,
            unit: this.language.log_requests,
            value: 0,
          },
        ];
      });
    ...
    this.logs.forEach((log: Log) => {
      ...
      statusGraph.values[log.get<Date>('date').getFullYear()][
        status.indexOf(log.get<string>('status'))
      ].value++;
      ...
      if (activeUsersGraph.values[log.get<string>('user')]) {
        ...
        activeUsersGraph.values[log.get<string>('user')] = [
          { name: log.get<string>('user'), value: 1, unit: 'requests' },
        ];
      }
    });
    ...
    activeUsersGraph.values = Object.keys(activeUsersGraph.values)
      .sort(
        (user1, user2) =>
          activeUsersGraph.values[user2][0].value -
          activeUsersGraph.values[user1][0].value
      )
      .reduce(
        (_sortedObj, key) => ({ ..._sortedObj, [key]: activeUsersGraph.values[key] }),
        {}
      );
