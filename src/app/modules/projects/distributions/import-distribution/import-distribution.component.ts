    ...
    if (this.userService.hasRights('ROLE_DISTRIBUTIONS_MANAGEMENT')) {
      ...
      if (this.csv && step === IMPORT_COMPARE) {
        ...
        this.beneficiaryService
          .import(this.distribution.get('id'), data, IMPORT_COMPARE)
          .subscribe(
            (result) => {
              this.comparing = true;
              this.loadFile = false;
              ...
              const createList = this.importedData.created.map((beneficiary: any) =>
                ImportedBeneficiary.apiToModel(beneficiary)
              );
              const addList = this.importedData.added.map((beneficiary: any) =>
                ImportedBeneficiary.apiToModel(beneficiary)
              );
              const removeList = this.importedData.deleted.map((beneficiary: any) =>
                ImportedBeneficiary.apiToModel(beneficiary)
              );
              const updateList = this.importedData.updated.map((beneficiary: any) =>
                ImportedBeneficiary.apiToModel(beneficiary)
              );
            ...
            (error) => {
              (this.loadFile = false), (this.csv = null);
            }
          );
        ...
        if (this.isJustified()) {
          ...
          this.beneficiaryService
            ...
            .subscribe(
              ...
              (error) => {
                (this.loadUpdate = false), (this.comparing = false);
              }
            );
        }
      }
  ...
  justify(importedBeneficiary: ImportedBeneficiary, template) {
    ...
    this.modalReference.afterClosed().subscribe((message: string) => {
      if (message === 'add') {
        ...
        const localFamilyName = importedBeneficiary
          .get('beneficiary')
          .get('localFamilyName');
        const localGivenName = importedBeneficiary
          .get('beneficiary')
          .get('localGivenName');

        const correspondingAdded = this.importedData.added.filter((added: any) => {
          return (
            added.localFamilyName === localFamilyName &&
            added.localGivenName === localGivenName
          );
        })[0];
        const correspondingCreated = this.importedData.created.filter((created: any) => {
          return (
            created.localFamilyName === localFamilyName &&
            created.localGivenName === localGivenName
          );
        })[0];
        const correspondingDeleted = this.importedData.deleted.filter((deleted: any) => {
          return (
            deleted.localFamilyName === localFamilyName &&
            deleted.localGivenName === localGivenName
          );
        })[0];
