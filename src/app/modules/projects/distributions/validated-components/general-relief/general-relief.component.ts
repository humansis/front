...
export class GeneralReliefComponent extends ValidatedDistributionComponent
  ...
  setDistributionBeneficiaries(distributionBeneficiaries: any) {
    this.actualDistribution.set(
      ...
      distributionBeneficiaries.map((distributionBeneficiary: any) =>
        TransactionGeneralRelief.apiToModel(
  ...
  distributeRelief() {
    ...
    this.selection.selected.forEach(
      (selectedDistributionBeneficiary: TransactionGeneralRelief) => {
        this.actualDistribution
          .get<TransactionGeneralRelief[]>('distributionBeneficiaries')
          .forEach((distributionBeneficiary: TransactionGeneralRelief) => {
            if (
              distributionBeneficiary.get('beneficiary').get('id') ===
              selectedDistributionBeneficiary.get('beneficiary').get('id')
            ) {
              distributionBeneficiary
                .get<GeneralRelief[]>('generalReliefs')
                .forEach((generalRelief: GeneralRelief) =>
    ...
    this.distributionService.distributeGeneralReliefs(generalReliefsId).subscribe(
      (result: any) => {
        ...
        this.selection.selected.forEach(
          (selectedDistributionBeneficiary: TransactionGeneralRelief) => {
            const distributionBeneficiaries = this.actualDistribution.get<
              TransactionGeneralRelief[]
            >('distributionBeneficiaries');
            distributionBeneficiaries.forEach(
              (distributionBeneficiary: TransactionGeneralRelief) => {
                if (
                  distributionBeneficiary.get('beneficiary').get('id') ===
                  selectedDistributionBeneficiary.get('beneficiary').get('id')
                ) {
                  const generalReliefs = distributionBeneficiary
                    .get<GeneralRelief[]>('generalReliefs')
  ...
  getCommoditySentAmountFromBeneficiary(
    commodity: Commodity,
    beneficiary: TransactionGeneralRelief
  ): number {
    const commodityInDistribution = this.actualDistribution
      .get<Commodity[]>('commodities')
      .filter(
        (commodityInList: Commodity) =>
          commodityInList.get('name') === commodity.get('name')
      )[0];
  ...
  updateElement(updateElement: TransactionGeneralRelief) {
    ...
    const generalReliefsForApi = generalReliefs.map((generalRelief: GeneralRelief) =>
      generalRelief.modelToApi()
    );
    ...
    const updateObservable = this.beneficiariesService.update(
      apiUpdateElement['id'],
      apiUpdateElement
    );
    ...
    forkJoin(addNotesObservable, updateObservable).subscribe(() => {
      ...
      distributionBeneficiaries.forEach(
        (distributionBeneficiary: TransactionGeneralRelief) => {
          if (
            distributionBeneficiary.get('beneficiary').get('id') ===
            updateElement.get('beneficiary').get('id')
          ) {
