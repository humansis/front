...
export class QrVoucherComponent extends ValidatedDistributionComponent implements OnInit {
  ...
  setDistributionBeneficiaries(distributionBeneficiaries: any) {
    this.actualDistribution.set(
      ...
      distributionBeneficiaries.map((distributionBeneficiariy: any) =>
        TransactionQRVoucher.apiToModel(
  ...
  formatTransactionTable() {
    const distributionBeneficiaries = this.actualDistribution.get<TransactionQRVoucher[]>(
      'distributionBeneficiaries'
    );
  ...
  getCommoditySentAmountFromBeneficiary(
    commodity: any,
    transaction: TransactionQRVoucher
  ): number {
  ...
  getCommodityReceivedAmountFromBeneficiary(
    ...
    if (
      booklet &&
      (booklet.get<CustomModel>('status').get<string>('id') === '2' ||
        booklet.get<CustomModel>('status').get<string>('id') === '3')
    ) {
      return booklet.get('value');
    } else {
      return 0;
  ...
  openAssignDialog() {
    ...
    this.distributionService
      .getAssignableBeneficiaries(this.actualDistribution.get('id'))
      .subscribe(
        (response) => {
          ...
          if (response || response === []) {
            ...
            beneficiaries = response.map((distributionBeneficiary: any) =>
