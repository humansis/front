...
export class AddDistributionComponent implements OnInit, DesactivationGuarded, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.getQueryParameter().subscribe((params) => {
      ...
      if (
        params.prefill === 'false' ||
        !this._distributionService.distributionToDuplicate
      ) {
  ...
  getProjectDates() {
    this._projectService.get().subscribe((projects) => {
      if (projects) {
  ...
  getMinDate() {
    return this.projectInfo.startDate > new Date()
  ...
  createElement(createElement: any, user_action: string) {
    if (user_action === 'addCriteria') {
      ...
      this.commodityData.data.forEach((commodity) => {
        this.commodityNb.push(
          commodity.get<number>('value') * this.criteriaNbBeneficiaries
        );
      });
    }
  }
  ...
  add() {
    if (
      ...
      this.criteriaData.data &&
      this.criteriaData.data.length !== 0 &&
      this.commodityData.data &&
      ...
      if (this.commodityData.data.length > 1) {
        const commodityAlone = this.commodityData.data.filter((commodity: Commodity) => {
          return (
            commodity.get<CustomModel>('modalityType').get<string>('name') ===
              'Mobile Money' ||
            commodity.get<CustomModel>('modalityType').get<string>('name') ===
              'QR Code Voucher'
          );
      ...
      newDistribution.set(
        'name',
        admName + '-' + datePipe.transform(this.form.controls.date.value, 'dd-MM-yyyy')
      );

      newDistribution.set(
        'type',
        this.objectInstance.getOptions('type').filter((option) => {
      ...
      this._distributionService.create(newDistribution.modelToApi()).subscribe(
        (response) => {
          if (response) {
