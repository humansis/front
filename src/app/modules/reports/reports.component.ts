...
export class ReportsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.getProjects()
      ...
      .subscribe((_: any) => {
        ...
        this.screenSizeService.displayTypeSource.subscribe(
  ...
  private generateFormsEvents() {
    this.subscriptions = [
      ...
      this.projectsControl.statusChanges
        .pipe(filter((event) => event === 'VALID'))
        .subscribe((_: any) => {
          this.onFilterChange();
        }),

      // Update distributions on project change in distribution report
      this.distributionsControl.controls.projectSelect.valueChanges
        .pipe(
      ...
      this.distributionsControl.statusChanges
        .pipe(filter((event) => event === 'VALID'))
        .subscribe((_: any) => {
          this.onFilterChange();
        }),
      // Send request when period report form is valid
      this.periodControl.statusChanges
        .pipe(filter((event) => event === 'VALID'))
        .subscribe((_: any) => {
          this.onFilterChange();
        }),
  ...
  private setFormsValues() {
    ...
    this.projectsControl.get('projectsSelect').setValue(
      this.projects.map((project: Project) => project.get('id')),
      { emitEvent: false }
    );
    const firstProjectId = this.projects[0].get<number>('id');
    this.distributionsControl
      .get('projectSelect')
      .setValue(firstProjectId, { emitEvent: false });
    ...
    return this.distributionService.getByProject(projectId).pipe(
      tap((apiDistributions: Array<any>) => {
        if (apiDistributions || apiDistributions === []) {
          ...
          this.distributionsControl.get('distributionsSelect').setValue(
            this.distributions.map((distribution: Distribution) => {
  ...
  selectReport(clickedReport: object) {
    ...
    this.selectedReport = selectedReports.length
      ? selectedReports[0]
      : this.enabledReports[0];
  ...
  selectFrequency(clickedFrequency: object) {
    ...
    this.selectedFrequency = selectedFrequencies.length
      ? selectedFrequencies[0]
      : undefined;
    this.onFilterChange();
  }
  ...
  private updateReports() {
    ...
    // Renew it
    this.getAllSubscription = this.indicatorService
      .getAllGraphs(this.generateFilters())
  ...
  private translateGraphs(graphDTO: GraphDTO): GraphDTO {
    graphDTO.name = this.language[
      'report_' + graphDTO.name.trim().replace(/ /g, '_').toLowerCase()
    ];
    if (graphDTO.values) {
      Object.keys(graphDTO.values).forEach((period: string) => {
        graphDTO.values[period].forEach((graphValue: GraphValueDTO) => {
          if (graphValue.unity && this.canTranslate(graphValue.unity)) {
            graphValue.unity = this.language[
              'report_' + graphValue.unity.trim().replace(/ /g, '_').toLowerCase()
            ];
          }
        });
      });
    ...
    switch (exportFileType) {
      ...
      case 'ods':
        this.indicatorService
          .exportReportData(this.generateFilters(), exportFileType)
          .subscribe((file: File) => {
  ...
  private generatePdf() {
    ...
    if (htmlGraphs.length > 0) {
      const graphWidthMm =
        (PDFConfig.paperWidthMm -
          (PDFConfig.graphPerLine + 1) * PDFConfig.singleMarginMm) /
        PDFConfig.graphPerLine;
          ...
          return from(html2canvas(htmlGraph, options)).pipe(
            tap((canvas: HTMLCanvasElement) => {
              ...
              pdf.addImage(
                img,
                'JPEG',
                x + PDFConfig.singleMarginMm,
                y + PDFConfig.singleMarginMm,
                graphWidthMm,
                graphHeightMm
              );
