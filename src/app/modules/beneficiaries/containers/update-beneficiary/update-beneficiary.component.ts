...
import {
  Beneficiary,
  BeneficiaryOptions,
  BeneficiaryReferralType,
  Gender,
} from 'src/app/models/beneficiary';
...
export class UpdateBeneficiaryComponent
  implements OnInit, DesactivationGuarded, OnDestroy {
  // Mode
  public mode: string;
  ...
  ngOnInit() {
    ...
    this.mode =
      this.router.url.split('/')[2] === 'update-beneficiary' ? 'update' : 'create';
      ...
      this.getVulnerabilityCriteria().subscribe((response) => {
        if (response) {
          ...
          this.household
            ...
            .forEach((beneficiary: Beneficiary, index: number) => {
  ...
  fillFormFields() {
    ...
    const vulnerabilityCriteriaNames = this.vulnerabilityList.map(
      (vulnerability: VulnerabilityCriteria) => {
  ...
  makeMainForm() {
    this.mainFields.forEach((fieldName: string) => {
      ...
      const field = this.household.fields[fieldName]
        ? this.household.fields[fieldName]
        : null;
      if (field && field.kindOfField === 'MultipleSelect') {
        const selectedOptions = this.household
          .get<CustomModel[]>(fieldName)
        ...
        this.mainForm.addControl(
          fieldName,
          new FormControl(field.value ? field.value.get(field.apiLabel) : null)
        );
      } else if (field && field.kindOfField === 'Date') {
        this.mainForm.addControl(
          fieldName,
          new FormControl(
            this.household.get(fieldName) ? new Date(this.household.get(fieldName)) : null
          )
        );
      } else {
        this.mainForm.addControl(
    ...
    this.mainForm.controls.livelihood.setValue(
      this.household.get('livelihood') ? this.household.get('livelihood').get('id') : null
    );
    this.makeLocationForm();

    this.countrySpecificService.get().subscribe((countrySpecifics: any) => {
      if (countrySpecifics) {
        this.countrySpecificList = countrySpecifics.map((countrySpecific: any) =>
          CountrySpecific.apiToModel(countrySpecific)
        );
        this.countrySpecificList.forEach((countrySpecific: CountrySpecific) => {
          this.mainFields.push(countrySpecific.get<string>('field'));
          const answer = this.household
            .get<CountrySpecificAnswer[]>('countrySpecificAnswers')
            .filter((countrySpecificAnswer) => {
              return (
                countrySpecificAnswer
                  .get<CustomModel>('countrySpecific')
                  .get<string>('field') === countrySpecific.get<string>('field')
              );
            })[0];
          this.mainForm.addControl(
            countrySpecific.get<string>('field'),
            new FormControl(answer ? answer.get<string>('answer') : null)
          );
          this.snapshot();
        });
      }
  ...
  makeLocationForm() {
    const householdLocations = [
      this.household.get<HouseholdLocation>('currentHouseholdLocation'),
    ];
    if (this.household.get<HouseholdLocation>('residentHouseholdLocation')) {
      householdLocations.push(
        this.household.get<HouseholdLocation>('residentHouseholdLocation')
      );
    }

    this.mainForm.controls.locationDifferent.setValue(
      householdLocations.length > 1 ? true : false,
      { emitEvent: false }
    );
    ...
    householdLocations.forEach((householdLocation: HouseholdLocation) => {
      const prefix =
        householdLocation.get('locationGroup') &&
      ...
      if (
        householdLocation.get('address') &&
        householdLocation.get('address').get('location')
      ) {
        locations[prefix] = householdLocation.get('address').get('location');
        this.mainForm.controls[prefix + 'AddressNumber'].setValue(
          householdLocation.get('address').get('number')
        );
        this.mainForm.controls[prefix + 'AddressStreet'].setValue(
          householdLocation.get('address').get('street')
        );
        this.mainForm.controls[prefix + 'AddressPostcode'].setValue(
          householdLocation.get('address').get('postcode')
        );
      } else if (
        householdLocation.get('campAddress') &&
        householdLocation.get('campAddress').get('camp') &&
        householdLocation.get('campAddress').get('camp').get('location')
      ) {
        locations[prefix] = householdLocation
          .get('campAddress')
          .get('camp')
          .get('location');
        this.mainForm.controls[prefix + 'TentNumber'].setValue(
          householdLocation.get('campAddress').get('tentNumber')
        );
      }
    });
    this.initialAdms['current'] = {
      currentAdm1: locations.current.get('adm1')
        ? locations.current.get<CustomModel>('adm1').get<number>('id')
        : null,
      currentAdm2: locations.current.get('adm2')
        ? locations.current.get<CustomModel>('adm2').get<number>('id')
        : null,
      currentAdm3: locations.current.get('adm3')
        ? locations.current.get<CustomModel>('adm3').get<number>('id')
        : null,
      currentAdm4: locations.current.get('adm4')
        ? locations.current.get<CustomModel>('adm4').get<number>('id')
        : null,
    };
    this.initialAdms['resident'] = {
      residentAdm1: locations.resident.get('adm1')
        ? locations.resident.get<CustomModel>('adm1').get<number>('id')
        : null,
      residentAdm2: locations.resident.get('adm2')
        ? locations.resident.get<CustomModel>('adm2').get<number>('id')
        : null,
      residentAdm3: locations.resident.get('adm3')
        ? locations.resident.get<CustomModel>('adm3').get<number>('id')
        : null,
      residentAdm4: locations.resident.get('adm4')
        ? locations.resident.get<CustomModel>('adm4').get<number>('id')
        : null,
    };
  }
  ...
  makeBeneficiaryForm(beneficiary: Beneficiary, index: number) {
    ...
    this.beneficiaryFields.forEach((fieldName: string) => {
      ...
      if (field && field.kindOfField === 'SingleSelect') {
        beneficiaryFormControls[fieldName] = new FormControl(
          beneficiary.get(fieldName) instanceof CustomModel
            ? beneficiary.get(fieldName).get('id')
            : null
        );
      } else {
        beneficiaryFormControls[fieldName] = new FormControl(
    ...
    ['localFamilyName', 'enFamilyName'].forEach((familyNameField: string) => {
      const headFamilyName = this.beneficiariesForm[0]
        ? this.beneficiariesForm[0].controls[familyNameField].value
        : null;
      if (
        beneficiaryFormControls[familyNameField] &&
        !beneficiaryFormControls[familyNameField].value &&
        headFamilyName
      ) {
    ...
    [0, 1].forEach((phoneIndex: number) => {
      ...
      if (phone) {
        beneficiaryFormControls['phoneType' + phoneIndex].setValue(
          phone.get('type') ? phone.get('type').get('id') : null
        );
        ...
        beneficiaryFormControls['phonePrefix' + phoneIndex].setValue(
          this.phoneService.getPhonePrefix(phone, this.countryId)
        );
      }
    });

    beneficiaryFormControls['IDType'].setValue(
      beneficiary.get<NationalId[]>('nationalIds')[0].get('type').get('id')
    );
    beneficiaryFormControls['IDNumber'].setValue(
      beneficiary.get<NationalId[]>('nationalIds')[0].get('number')
    );
    ...
    beneficiaryFormControls['residencyStatus'].setValue(
      beneficiary.get('residencyStatus')
        ? beneficiary.get('residencyStatus').get('id')
        : null
    );
  ...
  getBeneficiaryOptions(): BeneficiaryOptions {
    return {
      ...
      genderList: this.household
        .get<Beneficiary[]>('beneficiaries')[0]
        .getOptions('gender'),
      nationalIdList: this.household
        .get<Beneficiary[]>('beneficiaries')[0]
        .get<NationalId[]>('nationalIds')[0]
        .getOptions('type'),
      residencyStatusList: this.household
        .get<Beneficiary[]>('beneficiaries')[0]
        .getOptions('residencyStatus'),
      phoneList: this.household
        .get<Beneficiary[]>('beneficiaries')[0]
        .get<Phone[]>('phones')[0]
        .getOptions('type'),
      referralTypeList: this.household
        .get<Beneficiary[]>('beneficiaries')[0]
        .getOptions('referralType'),
    };
  }
  ...
  submit(step = null) {
    ...
    if (!step || this.nextValidation(step, true)) {
      ...
      this.household.set(
        'supportReceivedOtherOrg',
        this.household
          .getOptions('supportReceivedOtherOrg')
          .filter((supportOrg: IdNameModel) =>
            controls.supportReceivedOtherOrg.value.includes(supportOrg.get('id'))
          )
      );
      ...
      this.household.set(
        ...
        this.household
          .getOptions('assets')
          .filter((asset: IdNameModel) => controls.assets.value.includes(asset.get('id')))
      );
      this.household.set(
        'shelterStatus',
        controls.shelterStatus.value
          ? this.household
              .getOptions('shelterStatus')
              .filter(
                (shelterStatus: IdNameModel) =>
                  controls.shelterStatus.value === shelterStatus.get('id')
              )[0]
          : null
      );
      ...
      this.household.set(
        ...
        this.household
          ...
          .filter((project: Project) =>
            controls.projects.value.includes(project.get('id'))
          )
      );
      ...
      this.countrySpecificList.forEach((countrySpecific: CountrySpecific) => {
        ...
        countrySpecificAnswer.set(
          'answer',
          controls[countrySpecific.get<string>('field')].value
        );
      ...
      if (this.mode === 'create') {
        this._householdsService.create(body).subscribe(
          (_success: any) => {
        ...
        this._householdsService.update(this.household.get('id'), body).subscribe(
          (_success: any) => {
  ...
  fillLocationFromForm() {
    ...
    locationGroups.forEach((locationGroup: string) => {
      ...
      if (locationGroup === 'current' || controls.locationDifferent.value) {
        ...
        householdLocation.set(
          'locationGroup',
          householdLocation
            .getOptions('locationGroup')
            .filter(
              (option: HouseholdLocationGroup) =>
                option.get<string>('id') === locationGroup
            )[0]
        );
        ...
        householdLocation.set(
          ...
          householdLocation
            .getOptions('type')
            .filter(
              (option: HouseholdLocationType) => option.get<string>('id') === locationType
            )[0]
        );
        ...
        admIndexes.forEach((index) => {
          ...
          location.set(
            ...
            controls[formControlName].value
              ? location
                  .getOptions('adm' + index)
                  .filter(
                    (option: Adm) => option.get('id') === controls[formControlName].value
                  )[0]
              : null
          );
        });
        ...
        if (locationType !== 'camp') {
          ...
          if (controls[locationGroup + 'CreateCamp'].value) {
            ...
            camp = this.campLists[locationGroup].filter(
              (campFromList: Camp) =>
                campFromList.get<string>('id') === controls[locationGroup + 'Camp'].value
            )[0];
          }
    ...
    this.beneficiariesForm.forEach((form, index) => {
        ...
        ? this.household
            ...
            .filter(
              (householdBeneficiary) =>
                householdBeneficiary.get('id') === controls.id.value
            )[0]
      ...
      ['gender', 'residencyStatus'].forEach((field: string) => {
        beneficiary.set(
          ...
          beneficiary
            .getOptions(field)
            .filter((option) => option.get('id') === controls[field].value)[0]
        );
      });
      ...
      beneficiary.set(
        'beneficiaryStatus',
        index === 0 ? beneficiaryStatusOptions[1] : beneficiaryStatusOptions[0]
      );
      beneficiary.set(
        'vulnerabilities',
        this.vulnerabilityList.filter((vulnerability: VulnerabilityCriteria) => {
      ...
      nationalId.set(
        ...
        nationalId
          .getOptions('type')
          .filter(
            (option: NationalIdType) => controls.IDType.value === option.get('id')
          )[0]
      );
      ...
      [0, 1].forEach((phoneIndex: number) => {
        ...
        phone.set(
          ...
          phoneType
            ? phone
                .getOptions('type')
                .filter((type: PhoneType) => type.get('id') === phoneType)[0]
            : null
        );
      ...
      if (controls.referralComment.value && controls.referralType.value) {
        beneficiary.set(
          ...
          beneficiary
            .getOptions('referralType')
            .filter((referralType: BeneficiaryReferralType) => {
    ...
    locationGroups.forEach((locationGroup: string) => {
      if (locationGroup === 'current' || controls.locationDifferent.value) {
        if (!controls[locationGroup + 'Adm1'].value) {
          ...
          controls[locationGroup + 'Type'].value === 'camp' &&
          ((!controls[locationGroup + 'CreateCamp'].value &&
            !controls[locationGroup + 'Camp'].value) ||
            (controls[locationGroup + 'CreateCamp'].value &&
              !controls[locationGroup + 'NewCamp'].value))
        ) {
          locationMessage = this.language.beneficiary_error_camp;
        } else if (
          controls[locationGroup + 'Type'].value === 'camp' &&
          !controls.currentTentNumber.value
        ) {
  ...
  validateBeneficiaryForm(formIndex: number) {
    ...
    const numberSuffix =
      formIndex <= 3
        ? this.language.number_suffixes[formIndex]
        : this.language.number_suffix_other;
    const beneficiaryName =
      formIndex === 0
        ? this.language.beneficiary_error_head
        : this.language.the +
          ' ' +
          formIndex +
          numberSuffix +
          this.language.beneficiary_error_member;
    ...
    if (!beneficiary.localFamilyName.value) {
      ...
      (beneficiary.phoneNumber0.value &&
        beneficiary.phonePrefix0.value &&
        !this.countryCodesList.includes(beneficiary.phonePrefix0.value)) ||
      (beneficiary.phoneNumber1.value &&
        beneficiary.phonePrefix1.value &&
      ...
      (beneficiary.phoneNumber0.value &&
        (!beneficiary.phonePrefix0.value || beneficiary.phonePrefix0.value === '')) ||
      (beneficiary.phoneNumber1.value &&
        (!beneficiary.phonePrefix1.value || beneficiary.phonePrefix1.value === ''))
    ) {
      message = this.language.beneficiary_error_country_code + beneficiaryName;
    } else if (
      !beneficiary.dateOfBirth.value ||
      beneficiary.dateOfBirth.value.getTime() > new Date().getTime()
    ) {
    ...
    const adms = ['1', '2', '3', '4'].map((admIndex) => {
      ...
      return options
        ? options.filter(
            (option: Adm) =>
              option.get('id') ===
              this.mainForm.controls[locationGroup + 'Adm' + admIndex].value
          )[0]
        : null;
    });
  ...
  getGender(id: string) {
    return id
      ? this.household
          .get<Beneficiary[]>('beneficiaries')[0]
          .getOptions('gender')
          .filter((gender: Gender) => gender.get<string>('id') === id)[0]
          .get('name')
      : null;
  }
  ...
  getCampName(locationGroup) {
    ...
    return campId
      ? this.campLists[locationGroup]
          .filter((camp: Camp) => camp.get('id') === campId)[0]
          .get('name')
      : null;
  }
  ...
  getProjects() {
    this._projectService.get().subscribe((response) => {
      if (response) {
        this.household.setOptions(
          'projects',
          response.map((project) => Project.apiToModel(project))
        );
      }
    });
  }
    ...
    this._locationService.getCamps(event.admType, event.admId).subscribe((camps) => {
      if (camps) {
        ...
        if (
          ...
          const isInCampList = this.campLists[event.prefix].filter(
            (camp: Camp) =>
              camp.get('id') ===
              householdLocation.get('campAddress').get('camp').get('id')
          ).length;
          this.mainForm.controls[event.prefix + 'Camp'].setValue(
            isInCampList
              ? householdLocation.get('campAddress').get('camp').get('id')
              : null
          );
      ...
      map((response) => {
        if (response) {
          this.vulnerabilityList = response.map((criteria) =>
            VulnerabilityCriteria.apiToModel(criteria)
          );
        }
        return response;
      })
  ...
  private checkIfFormHasBeenModified(): boolean {
    ...
    const beneficiariesValue = this.beneficiariesForm.map(
      (beneficiaryForm) => beneficiaryForm.value
    );
  ...
  private beneficiarySnapshot(): void {
    this.uneditedBeneficiariesSnapshot = this.beneficiariesForm.map((beneficiaryForm) =>
      this.deepCopy(beneficiaryForm.value)
    );
  }
