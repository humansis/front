...
export class DataValidationComponent implements OnInit {
  ...
  public validateStep() {
    ...
    this.importService
      .sendStepUserData(this.generateResponse())
  ...
  public finishImport() {
    this.loading = true;
    this.importService.sendStepUserData(this.generateResponse()).subscribe(
      (response: any) => {
        this.loading = false;
        ...
        if (response) {
          this.importService.importedHouseholds = response.map((household: Household) =>
  ...
  private generateAddOrLessResponse(newOrOld: string): object {
    const response = this.errors.map((error: object, householdIndex: number) => {
      error[newOrOld].beneficiaries = error[newOrOld].beneficiaries.filter(
        (_beneficiaries: any, beneficiaryIndex: number) => {
  ...
  getOldRestoredBeneficiaries(householdIndex: number): object {
    return this.errors[householdIndex]['old'].beneficiaries.filter(
      (_beneficiary: any, beneficiaryIndex: number) => {
        if (beneficiaryIndex === 0) {
          return false;
        }
        return this.getValueFromIndexes(householdIndex, beneficiaryIndex);
