...
@Component({
  ...
  styleUrls: [
    './beneficiaries-import.component.scss',
    '../../../components/modals/modal.component.scss',
  ],
})
export class BeneficiariesImportComponent implements OnInit, OnDestroy {
  ...
  confirmConversion() {
    ...
    ['adm1', 'adm2', 'adm3', 'adm4'].forEach((admName: string) => {
      const filter = this.conversionLocation
        .getOptions(admName)
        .filter(
          (adm: Adm) => adm.get('id') === this.conversionForm.controls[admName].value
        );
   ...
   */
  getAPINames() {
    this.beneficiariesService.listApi().subscribe((response: object) => {
  ...
  listenForApiSelectorChanges() {
    this.apiSelectorSubscriber = this.apiForm
      .get('apiSelector')
      .valueChanges.subscribe((apiName: string) => {
  ...
  importHouseholdsFile() {
    if (!this.csv || !this.fileForm.controls['projects'].valid || this.load) {
      ...
      this.load = true;
      this.importService
        .sendCsv(this.csv, this.email, this.fileForm.controls['projects'].value)
        .subscribe(
          (response: any) => {
            if (response) {
  ...
  importHousholdsApi() {
    ...
    const params = {};
    this.selectedApi.parameters.forEach((parameter: ApiParameter) => {
      params[parameter.paramName] = this.apiForm.get([
        'parameters',
        parameter.paramName,
      ]).value;
    });
    ...
    this.beneficiariesService
      .importApi(body, this.apiForm.controls['projects'].value)
      .subscribe(
        (response) => {
          if (response) {
   ...
   */
  importedHouseholds() {
    this.householdsService.getImported(this.newHouseholds).subscribe((response) => {
      if (response) {
        this.newHouseholds = response.map((household: Household) =>
          Household.apiToModel(household)
        );
        this.snackbar.success(
          response.length + this.language.beneficiary_import_beneficiaries_imported
        );
      }
